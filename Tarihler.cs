    ...
    public partial class Tarihler : Form
    {
        ...
        public void Listele()
        {
            try
            {
                ...
                bag.Open();
                kmt = new MySqlCommand("select id as Sıra_No,tarih as Tarih from sinavtarihleri", bag);
                adp = new MySqlDataAdapter(kmt);
                adp.Fill(dt);
                dataGridView1.DataSource = dt;
                bag.Close();

                //Tablodaki Tarihin yanına hangi gün olduğunu yazan kod
        ...
        public void Kontrol(DateTime tarih1, DateTime tarih2)
        {
            ...
            if (tarihid == -1)
            {
                ...
                if (tarih1.ToShortDateString() != tarih2.ToShortDateString())//kullanıcı tarih aralığı belirlediyse tabloya ekleyecek,belirlemediyse belirlemesi istenecek
                {
                    ...
                    try
                    {
                        //Tabloda Aynı Tarih Olup Olmadığını Kontrol Edip var ise datatable a ekliyor
                        bag.Open();
                        kmt = new MySqlCommand(kontrol, bag);
                        adp = new MySqlDataAdapter(kmt);
                        dt.Clear(); // tablonun aktarıldığı clas temizlenir.
                        dataGridView1.DataSource = null; // datagridview temizlenir
                        dataGridView1.Columns.Clear();// datagridview temizlenir
                        ...
                        {
                            //ekranda seçilen aralıktaki tarihlerle yukarıda datatable a aktarılanı karşılaştırıyor ve aynı olmayan tarihleri aşağıda kullanılacak olan komut stringine atıyor
                            for (DateTime i = Convert.ToDateTime(tarih1.ToShortDateString()); i <= Convert.ToDateTime(tarih2.ToShortDateString()); i = i.AddDays(1)) // böyle yapılmasının sebebi saatler uyuşmadığı için saatleri sıfırlıyoruz
                            {
                    ...
                    mesaj = "Tarihler Kaydedildi. Lütfen Aşağıdaki Tablodan Seçtiğiniz Tarih Aralığındaki Milli Tatil Günlerini Çıkarınız!";
                ...
                switch (e.ColumnIndex)
                {
                    ...
                    case 3:
                        try
                        {
                            ...
                            {
                                bag.Open();
                                kmt = new MySqlCommand("DELETE FROM sinavtarihleri where id=" + tarihid + ";", bag);
                                kmt.ExecuteNonQuery();
                                bag.Close();
